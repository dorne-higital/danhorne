...
export default defineEventHandler(async (event): Promise<PageRecord> => {
	...
	const body = await readBody<{ id?: string; slug?: string; title?: string }>(event)
	...
	const { data, error } = await supabase
		.from('pages')
		.insert({ id: body.id, slug: body.slug, title: body.title, blocks: [] })
		.select('id, slug, title, blocks, updated_at')
		.single()
